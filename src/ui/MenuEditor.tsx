...
export function MenuEditor({ value, onChange }:{ value:any[], onChange:(v:any[])=>void }){
  ...
  const addItem=(i:number)=>{ const copy=value.slice(); copy[i].items.push({title:'Nuovo piatto',price:0,desc:''}); onChange(copy); };
  return (
    <View style={{ backgroundColor:'#0b1220', borderRadius:12, padding:12 }}>
      ...
      {value.map((sec,i)=>(
        <View key={i} style={{ marginBottom:12, backgroundColor:'#0f172a', borderRadius:12, padding:12 }}>
          <TextInput value={sec.section} onChangeText={(t)=>{ const c=value.slice(); c[i].section=t; onChange(c); }} style={{ backgroundColor:'#1f2937', color:'white', padding:10, borderRadius:10, marginBottom:8 }} />
          ...
          <TouchableOpacity onPress={()=>addItem(i)} style={{ backgroundColor:'#374151', padding:10, borderRadius:10, alignItems:'center' }}><Text style={{ color:'white' }}>Aggiungi piatto</Text></TouchableOpacity>
        </View>
      ))}
      <TouchableOpacity onPress={addSection} style={{ backgroundColor:'#22c55e', padding:12, borderRadius:12, alignItems:'center' }}><Text style={{ color:'white', fontWeight:'700' }}>Aggiungi sezione</Text></TouchableOpacity>
    </View>
  );
}
