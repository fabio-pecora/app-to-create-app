...
export default function Payment(){
  ...
  const openCheckout = async ()=>{
    if(!url){ Alert.alert('Config mancante','Imposta EXPO_PUBLIC_STRIPE_CHECKOUT_URL in .env'); return; }
    const returnScheme = process.env.EXPO_PUBLIC_RETURN_SCHEME || 'restappfactory';
    const returnUrl = `${returnScheme}://payment/success`;
    const finalUrl = `${url}?success_url=${encodeURIComponent(returnUrl)}&cancel_url=${encodeURIComponent(returnUrl)}`;
  ...
  return (
    <View style={{flex:1,backgroundColor:'#0f172a',padding:16,justifyContent:'center'}}>
      ...
      <TouchableOpacity onPress={openCheckout} style={{backgroundColor:'#22c55e',padding:14,borderRadius:12,alignItems:'center'}}><Text style={{color:'white',fontWeight:'700'}}>Paga ora</Text></TouchableOpacity>
    </View>
  );
}
