...
export default function Login(){
  ...
  return (
    <View style={{flex:1,backgroundColor:'#0f172a',padding:24,justifyContent:'center'}}>
      ...
      <TouchableOpacity onPress={onSubmit} style={{backgroundColor:'#22c55e',padding:14,borderRadius:12,alignItems:'center'}}><Text style={{color:'white',fontWeight:'700'}}>{mode==='login'?'Accedi':'Registrati'}</Text></TouchableOpacity>
      <TouchableOpacity onPress={()=>setMode(mode==='login'?'signup':'login')} style={{padding:14,alignItems:'center'}}><Text style={{color:'#93c5fd'}}>{mode==='login'?'Crea un account':'Hai già un account? Accedi'}</Text></TouchableOpacity>
