  ...
  return (
    <ScrollView style={{ flex:1, backgroundColor:'#0f172a' }} contentContainerStyle={{ padding:16 }}>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginBottom:12 }}>
        {templates.map(t => (
          <TemplateCard key={t.id} tpl={t} selected={selected===t.id} onPress={()=> setSelected(t.id)} />
        ))}
      </ScrollView>
      ...
      {tpl ? <Preview data={{ name, homeText, about, menu, template: tpl }} /> : null}

      <TouchableOpacity onPress={async()=> { await markIntentToPay({ name, homeText, about, menu, templateId: selected }); }} style={{ backgroundColor:'#374151', padding:14, borderRadius:12, alignItems:'center', marginTop:16 }}>
        <Text style={{ color:'white', fontWeight:'700' }}>Salva bozza</Text>
      </TouchableOpacity>

      <TouchableOpacity onPress={async()=> { await markIntentToPay({ name, homeText, about, menu, templateId: selected }); }} style={{ backgroundColor:'#22c55e', padding:14, borderRadius:12, alignItems:'center', marginTop:12, marginBottom:28 }}>
