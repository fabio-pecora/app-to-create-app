...
type Intent = { name:string; homeText:string; about:string; menu:any[]; templateId:string };
...
export function SessionProvider({ children }:{ children:React.ReactNode }){
  ...
  // inject template object for preview during runtime convenience
  const templates = require('../templates/templates').templates;
  const resolved = intentData ? { ...intentData, template: templates.find((t:any)=> t.id === intentData.templateId) } : null;

  return <Ctx.Provider value={{ user, profile, saveProfile, intentData: resolved ?? intentData, markIntentToPay }}>{children}</Ctx.Provider>;
}
